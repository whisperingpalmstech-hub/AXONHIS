...
export default function Page() {
  ...
  const load = useCallback(async () => {
    try { const data = await apiFetch("/audit/events"); setItems(data); } catch { setItems([]); }
    finally { setLoading(false); }
  }, []);
  useEffect(() => { load(); }, [load]);

  return (<div><TopNav title="Audit Trail" />
    <div className="p-6 space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div><h2 className="text-lg font-bold text-slate-800">Audit Trail</h2><p className="text-sm text-slate-500">{items.length} records</p></div>
      </div>
      ...
      items.length === 0 ? (
        <div className="text-center py-20">
          ...
          <h3 className="text-lg font-bold text-slate-700">No Audit Events</h3>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
        <table className="data-table"><thead><tr><th>#</th><th>Name / Identifier</th><th>Type / Category</th><th>Status</th><th>Created</th></tr></thead>
          <tbody>{items.map((item: any, i: number) => (
            <tr key={item.audit_event_id || i} className="hover:bg-teal-50/30">
              <td className="text-xs text-slate-400 font-mono">{i + 1}</td>
              <td className="font-semibold text-slate-800">{item.action_type || item.code || item.display_name || "—"}</td>
