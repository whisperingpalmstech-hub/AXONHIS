...
export default function QAPage() {
  ...
  const fetchModules = async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/v1/qa/modules?app=patient_portal");
  ...
  const testAllModules = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/v1/qa/test/all?app=patient_portal`, { method: "POST" });
      if (!response.ok) throw new Error(`HTTP ${response.status}`);
      const data = await response.json();
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
        <div>
          <h1 style={{ fontSize: "28px", fontWeight: 700, color: "#0f172a", margin: 0 }}>
            Patient Portal QA Testing Center
          </h1>
