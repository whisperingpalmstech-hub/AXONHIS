...
async def create_encounter(data: EncounterCreate, db: DBSession, user: CurrentUser):
    return await EncounterService(db).create_encounter(data, user.id)

@router.get("/", response_model=list[EncounterOut])
async def list_encounters(db: DBSession, _: CurrentUser, skip: int = 0, limit: int = 20):
    return await EncounterService(db).list_encounters(skip, limit)

@router.get("/{encounter_id}", response_model=EncounterOut)
async def get_encounter(encounter_id: uuid.UUID, db: DBSession, _: CurrentUser):
    encounter = await EncounterService(db).get_encounter_by_id(encounter_id)
    if not encounter:
