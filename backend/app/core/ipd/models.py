...
class IpdAdmissionRequest(Base):
    ...
    admission_category = Column(
        String, nullable=False
    )  # Emergency, Planned Admission, Day Care Admission
...
class IpdAdmissionRecord(Base):
    ...
    )  # e.g. IPD-ADM-2026-003541
    request_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ipd_admission_requests.id", ondelete="SET NULL"),
        nullable=True,
    )
    patient_uhid = Column(String, nullable=False, index=True)
...
class IpdAdmissionAuditLog(Base):
    ...
    action_type = Column(
        String, nullable=False
    )  # Request Created, Bed Allocated, Admission Approved, Discharged
...
class IpdCostEstimation(Base):
    ...
    request_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ipd_admission_requests.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
...
class IpdPatientStatusMonitor(Base):
    ...
    status = Column(
        String, default="Pending Acceptance"
    )  # Pending Acceptance, Accepted, Under Observation, Under Treatment, Transferred, Discharged
...
class IpdNursingNote(Base):
    ...
    note_type = Column(
        String, default="Initial Assessment"
    )  # e.g. Initial Assessment, Shift Handover
...
class IpdTreatmentPlan(Base):
    ...
    therapy_type = Column(
        String, nullable=False
    )  # Medication, Procedure, Investigation, Supportive
...
class IpdOrdersMaster(Base):
    ...
    order_type = Column(
        String, nullable=False
    )  # Laboratory, Radiology, Medication, Procedure, Nursing
    ...
    status = Column(
        String, default="Ordered"
    )  # Ordered, Scheduled, In Progress, Completed, Cancelled
...
class IpdLabOrder(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    order_id = Column(
        UUID(as_uuid=True), ForeignKey("ipd_orders_master.id", ondelete="CASCADE"), nullable=False
    )
    ...
    )  # Pending, Sample Collected, Processing, Result Available
...
class IpdTransferRecord(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    transfer_request_id = Column(
        UUID(as_uuid=True),
        ForeignKey("ipd_transfer_requests.id", ondelete="CASCADE"),
        nullable=False,
    )
    admission_number = Column(String, nullable=False)
    patient_uhid = Column(String, nullable=False)
...
class IpdBillingService(Base):
    ...
    service_category = Column(
        String, nullable=False
    )  # Bed, Lab, Radiology, Pharmacy, Procedure, Nursing, Consumable
...
class IpdPaymentTransaction(Base):
    ...
    payment_mode = Column(
        String, nullable=False
    )  # Cash, Credit Card, Debit Card, Cheque, Bank Transfer, Online
    reference_number = Column(String, nullable=True)
    receipt_number = Column(String, nullable=True)
...
class IpdVisitorPass(Base):
    ...
    visitor_id = Column(
        UUID(as_uuid=True), ForeignKey("ipd_visitors.id", ondelete="CASCADE"), nullable=False
    )
    admission_number = Column(
        String,
        ForeignKey("ipd_admission_records.admission_number", ondelete="CASCADE"),
...
class IpdVisitorLog(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    pass_number = Column(
        String, ForeignKey("ipd_visitor_passes.pass_number", ondelete="CASCADE"), nullable=False
    )
...
class IpdMlcDocument(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    mlc_id = Column(
        UUID(as_uuid=True), ForeignKey("ipd_mlc_cases.id", ondelete="CASCADE"), nullable=False
    )
