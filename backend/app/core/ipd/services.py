...
from .models import (
    ...
    IpdProjectedBill, IpdDepositRecord, IpdPatientTransport,
...
class IPDService:
    ...
    async def log_audit(self, action: str, entity_id: str, uhid: str | None, details: dict, user_id: UUID | None = None):
    ...
    # --- Admission Requests --- #
    async def create_admission_request(self, data: dict, user_id: UUID | None = None) -> IpdAdmissionRequest:
        req_raw = data.copy()
    ...
    async def get_pending_requests(self) -> list[IpdAdmissionRequest]:
        result = await self.db.execute(
            select(IpdAdmissionRequest).where(IpdAdmissionRequest.status == "Pending")
        )
        return result.scalars().all()

    async def get_request_by_id(self, req_id: UUID) -> IpdAdmissionRequest | None:
        result = await self.db.execute(
            select(IpdAdmissionRequest).where(IpdAdmissionRequest.id == req_id)
        )
        return result.scalars().first()

    async def update_request_status(self, req_id: UUID, status: str, user_id: UUID | None = None) -> IpdAdmissionRequest | None:
    ...
    async def get_dashboard_stats(self) -> dict:
        ...
        # Total Beds
        tb_res = await self.db.execute(select(func.count(Bed.id)))
        total_beds = tb_res.scalar() or 0
        
        # Status Counts
        st_res = await self.db.execute(select(Bed.status, func.count(Bed.id)).group_by(Bed.status))
        status_counts = dict(st_res.all())
        
        # Pending Requests
        pr_res = await self.db.execute(select(func.count(IpdAdmissionRequest.id)).where(IpdAdmissionRequest.status == "Pending"))
        pending_requests = pr_res.scalar() or 0
    ...
    async def allocate_bed(self, req_id: UUID, bed_code: str, user_id: UUID | None = None) -> IpdAdmissionRecord | None:
        from app.core.wards.models import Bed
        
        # Load request
        req = await self.get_request_by_id(req_id)
        if not req or req.status != "Approved": return None
        
        # Load bed from central Wards module
        res = await self.db.execute(select(Bed).where(Bed.bed_code == bed_code))
        bed = res.scalars().first()
        if not bed or bed.status not in ["available", "reserved"]: return None
    ...
    async def get_active_admissions(self) -> list['IpdAdmissionRecord']:
        res = await self.db.execute(select(IpdAdmissionRecord).order_by(IpdAdmissionRecord.admission_time.desc()))
    ...
    async def get_checklist(self, adm_no: str) -> Optional[IpdAdmissionChecklist]:
        ...
        res = await self.db.execute(select(IpdAdmissionChecklist).where(IpdAdmissionChecklist.admission_number == adm_no))
    ...
    async def update_checklist(self, adm_no: str, data: dict) -> Optional[IpdAdmissionChecklist]:
        ...
        # check if all true
        required_fields = ["registration_completed", "identity_proof_verified", "consent_taken", "deposit_collected"]
    ...
    async def get_projected_bill(self, adm_no: str) -> Optional[IpdProjectedBill]:
        ...
        res = await self.db.execute(select(IpdProjectedBill).where(IpdProjectedBill.admission_number == adm_no))
    ...
    async def get_nursing_worklist(self) -> list['IpdNursingWorklist']:
        ...
        res = await self.db.execute(select(IpdNursingWorklist).order_by(IpdNursingWorklist.admission_time.desc()))
        return res.scalars().all()
        
    async def accept_patient(self, adm_no: str, priority_status: str, nurse_id: str, nurse_name: str) -> 'IpdNursingCoversheet':
        from .models import IpdNursingCoversheet, IpdNursingWorklist, IpdAdmissionRecord, IpdPatientStatusMonitor
        ...
        # 2. Check worklist
        wl_res = await self.db.execute(select(IpdNursingWorklist).where(IpdNursingWorklist.admission_number == adm_no))
        wl = wl_res.scalars().first()
        if wl:
            wl.status = "Accepted"
            
        # 3. Create or update coversheet
        cs_res = await self.db.execute(select(IpdNursingCoversheet).where(IpdNursingCoversheet.admission_number == adm_no))
        ...
        if not cs:
            cs = IpdNursingCoversheet(
                ...
                accepted_by_nurse_id=UUID(nurse_id) if (nurse_id and nurse_id != "00000000-0000-0000-0000-000000000000") else None,
                accepted_by_nurse_name=nurse_name
            )
            self.db.add(cs)
        else:
            cs.acceptance_time = datetime.now(timezone.utc)
            cs.accepted_by_nurse_id = UUID(nurse_id) if (nurse_id and nurse_id != "00000000-0000-0000-0000-000000000000") else None
            cs.accepted_by_nurse_name = nurse_name
            cs.priority_status = priority_status
            
        # 4. Update status monitor
        stat_res = await self.db.execute(select(IpdPatientStatusMonitor).where(IpdPatientStatusMonitor.admission_number == adm_no))
    ...
    async def add_nursing_note(self, admission_number: str, note_type: str, clinical_note: str, user_id: str, user_name: str) -> 'IpdNursingNote':
        from .models import IpdNursingNote
        note = IpdNursingNote(
            admission_number=admission_number,
            note_type=note_type,
            clinical_note=clinical_note,
            logged_by_id=UUID(user_id) if (user_id and user_id != "00000000-0000-0000-0000-000000000000") else uuid.uuid4(),
    ...
    async def record_vitals(self, adm_no: str, data: dict, user_name: str = "Nurse") -> IpdVitalsRecord:
        adm_res = await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))
        adm = adm_res.scalars().first()
        if not adm:
            raise ValueError("Admission not found")
    ...
    async def save_nursing_assessment(self, adm_no: str, data: dict, user_name: str = "Nurse") -> IpdNursingAssessment:
        adm_res = await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))
        adm = adm_res.scalars().first()
        if not adm:
            raise ValueError("Admission not found")
        ...
        # Upsert
        existing = await self.db.execute(select(IpdNursingAssessment).where(IpdNursingAssessment.admission_number == adm_no))
    ...
    async def get_nursing_assessment(self, adm_no: str) -> Optional[IpdNursingAssessment]:
        res = await self.db.execute(select(IpdNursingAssessment).where(IpdNursingAssessment.admission_number == adm_no))
        return res.scalars().first()

    async def add_risk_assessment(self, adm_no: str, data: dict, user_name: str = "Nurse") -> IpdRiskAssessment:
    ...
    async def get_risk_assessments(self, adm_no: str) -> list[IpdRiskAssessment]:
        res = await self.db.execute(select(IpdRiskAssessment).where(IpdRiskAssessment.admission_number == adm_no).order_by(IpdRiskAssessment.assessed_at.desc()))
        return res.scalars().all()

    async def add_pain_score(self, adm_no: str, data: dict, user_name: str = "Nurse") -> IpdPainScore:
        ps = IpdPainScore(admission_number=adm_no, recorded_by_name=user_name, **data)
        self.db.add(ps)
        await self.db.flush()
        return ps

    async def add_nutrition_assessment(self, adm_no: str, data: dict, user_name: str = "Nurse") -> IpdNutritionAssessment:
    ...
    async def add_observation(self, adm_no: str, data: dict, user_name: str = "Nurse") -> IpdNursingObservation:
    ...
    async def get_observations(self, adm_no: str):
        ...
        res = await self.db.execute(select(IpdNursingObservation).where(IpdNursingObservation.admission_number == adm_no).order_by(IpdNursingObservation.recorded_at.desc()))
        return res.scalars().all()
    ...
    async def get_doctor_worklist(self):
        ...
        admissions = await self.db.execute(select(IpdAdmissionRecord).where(
            IpdAdmissionRecord.status.in_(["Admitted", "Pending Acceptance", "Accepted", "Under Observation", "Active"]),
            ~IpdAdmissionRecord.admission_number.in_(select(IpdDoctorWorklist.admission_number))
        ))
        ...
        res = await self.db.execute(select(IpdDoctorWorklist).order_by(IpdDoctorWorklist.assigned_at.desc()))
        return res.scalars().all()

    async def get_doctor_coversheet(self, adm_no: str):
        from .models import IpdDoctorCoversheet, IpdAdmissionRecord
        res = await self.db.execute(select(IpdDoctorCoversheet).where(IpdDoctorCoversheet.admission_number == adm_no))
        coversheet = res.scalars().first()
        if not coversheet:
            # Create a blank coversheet automatically if it doesn't exist
            adm = await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))
    ...
    async def get_diagnoses(self, adm_no: str):
        ...
        res = await self.db.execute(select(IpdDiagnosis).where(IpdDiagnosis.admission_number == adm_no).order_by(IpdDiagnosis.diagnosed_at.desc()))
        return res.scalars().all()
    ...
    async def get_treatment_plans(self, adm_no: str):
        ...
        res = await self.db.execute(select(IpdTreatmentPlan).where(IpdTreatmentPlan.admission_number == adm_no).order_by(IpdTreatmentPlan.created_at.desc()))
        return res.scalars().all()
    ...
    async def get_progress_notes(self, adm_no: str):
        ...
        res = await self.db.execute(select(IpdProgressNote).where(IpdProgressNote.admission_number == adm_no).order_by(IpdProgressNote.recorded_at.desc()))
        return res.scalars().all()
    ...
    async def get_clinical_procedures(self, adm_no: str):
        ...
        res = await self.db.execute(select(IpdClinicalProcedure).where(IpdClinicalProcedure.admission_number == adm_no).order_by(IpdClinicalProcedure.procedure_date.desc()))
        return res.scalars().all()
    ...
    async def get_consultation_requests(self, adm_no: str):
        ...
        res = await self.db.execute(select(IpdConsultationRequest).where(IpdConsultationRequest.admission_number == adm_no).order_by(IpdConsultationRequest.requested_at.desc()))
        return res.scalars().all()
    ...
    async def create_lab_order(self, adm_no: str, data: dict, doctor_name: str = "Doctor"):
        ...
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))).scalar_one_or_none()
        if not adm:
            return None
        priority = data.pop("priority", "Routine")
        master = IpdOrdersMaster(
            admission_number=adm_no, patient_uhid=adm.patient_uhid,
            doctor_name=doctor_name, order_type="Laboratory", priority=priority
        )
        self.db.add(master)
        await self.db.flush()
    ...
    async def create_radiology_order(self, adm_no: str, data: dict, doctor_name: str = "Doctor"):
        from .models import IpdOrdersMaster, IpdRadiologyOrder, IpdOrderStatusLog, IpdAdmissionRecord
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))).scalar_one_or_none()
        if not adm:
            return None
        priority = data.pop("priority", "Routine")
        master = IpdOrdersMaster(
            admission_number=adm_no, patient_uhid=adm.patient_uhid,
            doctor_name=doctor_name, order_type="Radiology", priority=priority
        )
        self.db.add(master)
        await self.db.flush()
    ...
    async def create_medication_order(self, adm_no: str, data: dict, doctor_name: str = "Doctor"):
        from .models import IpdOrdersMaster, IpdMedicationOrder, IpdOrderStatusLog, IpdAdmissionRecord
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))).scalar_one_or_none()
        if not adm:
            return None
        priority = data.pop("priority", "Routine")
        master = IpdOrdersMaster(
            admission_number=adm_no, patient_uhid=adm.patient_uhid,
            doctor_name=doctor_name, order_type="Medication", priority=priority
        )
        self.db.add(master)
        await self.db.flush()
    ...
    async def create_procedure_order(self, adm_no: str, data: dict, doctor_name: str = "Doctor"):
        from .models import IpdOrdersMaster, IpdProcedureOrder, IpdOrderStatusLog, IpdAdmissionRecord
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))).scalar_one_or_none()
        if not adm:
            return None
        priority = data.pop("priority", "Routine")
        master = IpdOrdersMaster(
            admission_number=adm_no, patient_uhid=adm.patient_uhid,
            doctor_name=doctor_name, order_type="Procedure", priority=priority
        )
        self.db.add(master)
        await self.db.flush()
    ...
    async def get_patient_orders(self, adm_no: str):
        ...
        res = await self.db.execute(
            select(IpdOrdersMaster).where(IpdOrdersMaster.admission_number == adm_no)
            .order_by(IpdOrdersMaster.order_date.desc())
        )
        return res.scalars().all()

    async def get_order_detail(self, order_id):
        from .models import (IpdOrdersMaster, IpdLabOrder, IpdRadiologyOrder,
                             IpdMedicationOrder, IpdProcedureOrder, IpdOrderStatusLog)
        order = (await self.db.execute(select(IpdOrdersMaster).where(IpdOrdersMaster.id == order_id))).scalar_one_or_none()
        if not order:
            return None
        lab = (await self.db.execute(select(IpdLabOrder).where(IpdLabOrder.order_id == order_id))).scalar_one_or_none()
        rad = (await self.db.execute(select(IpdRadiologyOrder).where(IpdRadiologyOrder.order_id == order_id))).scalar_one_or_none()
        med = (await self.db.execute(select(IpdMedicationOrder).where(IpdMedicationOrder.order_id == order_id))).scalar_one_or_none()
        proc = (await self.db.execute(select(IpdProcedureOrder).where(IpdProcedureOrder.order_id == order_id))).scalar_one_or_none()
        logs = (await self.db.execute(select(IpdOrderStatusLog).where(IpdOrderStatusLog.order_id == order_id).order_by(IpdOrderStatusLog.updated_at.desc()))).scalars().all()
        return {
            "order": order,
            "lab_detail": lab,
    ...
    async def update_order_status(self, order_id, new_status: str, user_name: str = "System", remarks: str = None):
        ...
        log = IpdOrderStatusLog(order_id=order_id, previous_status=prev, new_status=new_status, updated_by=user_name, remarks=remarks)
        self.db.add(log)
        await self.db.flush()
        return order
    ...
    async def get_nursing_execution_worklist(self):
        from .models import IpdOrdersMaster
        res = await self.db.execute(
            select(IpdOrdersMaster).where(IpdOrdersMaster.status.in_(["Ordered", "Scheduled", "In Progress"]))
            .order_by(IpdOrdersMaster.order_date.desc())
        )
        return res.scalars().all()
    ...
    async def create_transfer_request(self, adm_no: str, data: dict, user_name: str = "Doctor"):
        from .models import IpdTransferRequest, IpdTransferAuditLog, IpdTransferHandover, IpdAdmissionRecord
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == adm_no))).scalar_one_or_none()
        if not adm:
            return None
        ...
        if handover_data:
            ho = IpdTransferHandover(
                transfer_request_id=req.id, admission_number=adm_no,
                documented_by=user_name, **handover_data
            )
            self.db.add(ho)
            
        log = IpdTransferAuditLog(
            transfer_request_id=req.id, action="Created", performed_by=user_name,
    ...
    async def get_transfer_requests(self, ward: str = None):
        ...
        if ward:
            ...
            query = query.where((IpdTransferRequest.current_ward == ward) | (IpdTransferRequest.requested_ward == ward))
    ...
    async def get_transfer_request_detail(self, req_id):
        ...
        req = (await self.db.execute(select(IpdTransferRequest).where(IpdTransferRequest.id == req_id))).scalar_one_or_none()
        if not req:
            return None
        handover = (await self.db.execute(select(IpdTransferHandover).where(IpdTransferHandover.transfer_request_id == req_id))).scalar_one_or_none()
        logs = (await self.db.execute(select(IpdTransferAuditLog).where(IpdTransferAuditLog.transfer_request_id == req_id).order_by(IpdTransferAuditLog.performed_at.desc()))).scalars().all()
    ...
    async def approve_transfer_request(self, req_id: str, approved_bed: str, user_name: str, remarks: str = None):
        from .models import IpdTransferRequest, IpdTransferAuditLog, IpdTransferRecord, IpdBedMovement, IpdAdmissionRecord
        req = (await self.db.execute(select(IpdTransferRequest).where(IpdTransferRequest.id == req_id))).scalar_one_or_none()
        ...
        record = IpdTransferRecord(
            transfer_request_id=req.id, admission_number=req.admission_number, patient_uhid=req.patient_uhid,
            from_ward=req.current_ward, from_bed=req.current_bed, to_ward=req.requested_ward, to_bed=approved_bed,
            transfer_type=req.transfer_type, approved_by=user_name
        )
        self.db.add(record)
        
        # Create bed movement
        movement = IpdBedMovement(
            admission_number=req.admission_number, patient_uhid=req.patient_uhid,
            previous_ward=req.current_ward, previous_bed=req.current_bed,
            new_ward=req.requested_ward, new_bed=approved_bed, movement_reason=req.reason,
            recorded_by=user_name
        )
        self.db.add(movement)
        
        # Update admission record
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == req.admission_number))).scalar_one_or_none()
        if adm:
            adm.ward_id = req.requested_ward
            adm.bed_number = approved_bed
        
        log = IpdTransferAuditLog(
            transfer_request_id=req.id, action="Approved", performed_by=user_name,
    ...
    async def reject_transfer_request(self, req_id: str, user_name: str, remarks: str = None):
        ...
        req.status = "Rejected"
        log = IpdTransferAuditLog(
            transfer_request_id=req.id, action="Rejected", performed_by=user_name,
    ...
    async def get_or_create_discharge_state(self, admission_number: str):
        from .models import IpdAdmissionRecord, IpdDischargePlan, IpdDischargeChecklist, IpdDischargeSummary
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == admission_number))).scalar_one_or_none()
        if not adm:
            return None
        
        plan = (await self.db.execute(select(IpdDischargePlan).where(IpdDischargePlan.admission_number == admission_number))).scalar_one_or_none()
        if not plan:
            plan = IpdDischargePlan(admission_number=adm.admission_number, patient_uhid=adm.patient_uhid, doctor_uuid=adm.admitting_doctor)
            self.db.add(plan)
        
        checklist = (await self.db.execute(select(IpdDischargeChecklist).where(IpdDischargeChecklist.admission_number == admission_number))).scalar_one_or_none()
        if not checklist:
            checklist = IpdDischargeChecklist(admission_number=adm.admission_number, patient_uhid=adm.patient_uhid)
            self.db.add(checklist)
        
        summary = (await self.db.execute(select(IpdDischargeSummary).where(IpdDischargeSummary.admission_number == admission_number))).scalar_one_or_none()
        if not summary:
            summary = IpdDischargeSummary(admission_number=adm.admission_number, patient_uhid=adm.patient_uhid, doctor_uuid=adm.admitting_doctor)
            self.db.add(summary)
    ...
    async def update_discharge_plan(self, admission_number: str, data: dict, user_name: str):
        ...
        log = IpdDischargeAuditLog(admission_number=admission_number, action="Plan Updated", performed_by=user_name, details=f"Planned discharge date updated: {plan.planned_discharge_date}")
        self.db.add(log)
        await self.db.flush()
        return plan
    ...
    async def update_discharge_checklist(self, admission_number: str, data: dict, user_name: str):
        ...
        if details:
            log = IpdDischargeAuditLog(admission_number=admission_number, action="Checklist Updated", performed_by=user_name, details=", ".join(details))
            self.db.add(log)
            await self.db.flush()
        return checklist
    ...
    async def generate_discharge_summary(self, admission_number: str, user_name: str):
        ...
        # Aggregate diagnoses
        diagnoses_records = (await self.db.execute(select(IpdDiagnosis).where(IpdDiagnosis.admission_number == admission_number))).scalars().all()
        diag_text = ", ".join([d.diagnosis_name for d in diagnoses_records]) if diagnoses_records else "Admitting Diagnosis"
        
        # Aggregate completed procedures
        procedures_records = (await self.db.execute(select(IpdOrdersMaster).where(
            IpdOrdersMaster.admission_number == admission_number, 
            IpdOrdersMaster.order_type == "Procedure",
            IpdOrdersMaster.status == "Completed"
        ))).scalars().all()
        proc_text = ", ".join([p.priority for p in procedures_records]) if procedures_records else "None recorded"
        ...
        log = IpdDischargeAuditLog(admission_number=admission_number, action="Summary Auto-Generated", performed_by=user_name, details="Generated from patient records")
        self.db.add(log)
        await self.db.flush()
        return summary
    ...
    async def update_discharge_summary(self, admission_number: str, data: dict, user_name: str):
        ...
        log = IpdDischargeAuditLog(admission_number=admission_number, action="Summary Updated", performed_by=user_name, details="Manual updates applied")
        self.db.add(log)
        await self.db.flush()
        return summary
    ...
    async def validate_pending_orders(self, admission_number: str):
        from .models import IpdOrdersMaster
        orders = (await self.db.execute(select(IpdOrdersMaster).where(
            IpdOrdersMaster.admission_number == admission_number,
    ...
    async def finalize_discharge(self, admission_number: str, user_name: str):
        from .models import IpdAdmissionRecord, IpdDischargePlan, IpdDischargeChecklist, IpdDischargeAuditLog
        from app.core.wards.models import Bed
        state = await self.get_or_create_discharge_state(admission_number)
        if not state:
            raise ValueError("Admission not found")
        ...
        checklist = state["checklist"]
        if not (checklist.doctor_approval and checklist.nursing_clearance and checklist.billing_clearance):
            raise ValueError("All clearances (Doctor, Nursing, Billing) must be approved to discharge.")
        ...
        bed = (await self.db.execute(select(Bed).where(Bed.id == adm.bed_uuid))).scalar_one_or_none()
        if bed:
            bed.status = "cleaning"
        ...
        log = IpdDischargeAuditLog(admission_number=admission_number, action="Discharge Finalized", performed_by=user_name, details="Patient physically discharged and bed freed.")
        self.db.add(log)
        await self.db.flush()
        return True
    ...
    async def get_or_create_billing(self, admission_number: str):
        ...
        billing = (await self.db.execute(select(IpdBillingMaster).where(IpdBillingMaster.admission_number == admission_number))).scalar_one_or_none()
        if billing:
            return billing
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == admission_number))).scalar_one_or_none()
        if not adm:
        ...
        if adm.request_id:
            ...
            req = (await self.db.execute(select(IpdAdmissionRequest).where(IpdAdmissionRequest.id == adm.request_id))).scalar_one_or_none()
        ...
        billing = IpdBillingMaster(
            ...
            patient_name=patient_name, ward_name="IPD Ward", bed_code=str(adm.bed_uuid) if adm.bed_uuid else None
        )
        self.db.add(billing)
        await self.db.flush()
        return billing
    ...
    async def recalculate_billing(self, admission_number: str):
        from .models import IpdBillingMaster, IpdBillingService, IpdBillingDeposit, IpdPaymentTransaction, IpdInsuranceClaim
        billing = await self.get_or_create_billing(admission_number)
        if not billing:
            return None
        services = (await self.db.execute(select(IpdBillingService).where(IpdBillingService.admission_number == admission_number))).scalars().all()
        billing.total_charges = sum(s.total_price for s in services)
        deposits = (await self.db.execute(select(IpdBillingDeposit).where(IpdBillingDeposit.admission_number == admission_number, IpdBillingDeposit.status == "Collected"))).scalars().all()
        billing.total_deposits = sum(d.amount for d in deposits)
        payments = (await self.db.execute(select(IpdPaymentTransaction).where(IpdPaymentTransaction.admission_number == admission_number, IpdPaymentTransaction.payment_type != "Refund"))).scalars().all()
        billing.total_paid = sum(p.amount for p in payments)
        claims = (await self.db.execute(select(IpdInsuranceClaim).where(IpdInsuranceClaim.admission_number == admission_number, IpdInsuranceClaim.status == "Approved"))).scalars().all()
    ...
    async def get_billing_services(self, admission_number: str):
        ...
        return (await self.db.execute(select(IpdBillingService).where(IpdBillingService.admission_number == admission_number).order_by(IpdBillingService.service_date.desc()))).scalars().all()
    ...
    async def add_billing_service(self, admission_number: str, data: dict, user_name: str):
        ...
        svc = IpdBillingService(
            admission_number=admission_number, service_category=data["service_category"],
            service_name=data["service_name"], quantity=qty, unit_price=unit_price,
            total_price=round(qty * unit_price, 2), notes=data.get("notes")
        )
        self.db.add(svc)
        log = IpdBillingAuditLog(admission_number=admission_number, action="Service Added", performed_by=user_name, details=f"{data['service_name']} ₹{svc.total_price}")
        self.db.add(log)
        await self.db.flush()
    ...
    async def add_deposit(self, admission_number: str, data: dict, user_name: str):
        ...
        dep = IpdBillingDeposit(
            admission_number=admission_number, patient_uhid=adm.patient_uhid,
            amount=data["amount"], payment_mode=data["payment_mode"],
            receipt_number=receipt, reference_number=data.get("reference_number"),
            collected_by=user_name
        )
        self.db.add(dep)
        log = IpdBillingAuditLog(admission_number=admission_number, action="Deposit Collected", performed_by=user_name, details=f"₹{data['amount']} via {data['payment_mode']}")
        self.db.add(log)
        await self.db.flush()
        await self.recalculate_billing(admission_number)
        return dep
    ...
    async def get_deposits(self, admission_number: str):
        ...
        return (await self.db.execute(select(IpdBillingDeposit).where(IpdBillingDeposit.admission_number == admission_number).order_by(IpdBillingDeposit.deposit_date.desc()))).scalars().all()
    ...
    async def add_insurance_claim(self, admission_number: str, data: dict, user_name: str):
        ...
        claim = IpdInsuranceClaim(
            ...
            pre_auth_number=data.get("pre_auth_number"), coverage_limit=data.get("coverage_limit", 0),
            claimed_amount=data.get("claimed_amount", 0)
        )
        self.db.add(claim)
        log = IpdBillingAuditLog(admission_number=admission_number, action="Insurance Claim Created", performed_by=user_name, details=f"{data['insurance_provider']} - ₹{data.get('claimed_amount', 0)}")
        self.db.add(log)
        await self.db.flush()
        return claim
    ...
    async def get_insurance_claims(self, admission_number: str):
        ...
        return (await self.db.execute(select(IpdInsuranceClaim).where(IpdInsuranceClaim.admission_number == admission_number))).scalars().all()
    ...
    async def process_payment(self, admission_number: str, data: dict, user_name: str):
        ...
        txn = IpdPaymentTransaction(
            admission_number=admission_number, patient_uhid=adm.patient_uhid,
            amount=data["amount"], payment_mode=data["payment_mode"],
            reference_number=data.get("reference_number"), receipt_number=receipt,
            processed_by=user_name
        )
        self.db.add(txn)
        log = IpdBillingAuditLog(admission_number=admission_number, action="Payment Received", performed_by=user_name, details=f"₹{data['amount']} via {data['payment_mode']}")
        self.db.add(log)
        await self.db.flush()
        await self.recalculate_billing(admission_number)
        return txn
    ...
    async def get_payments(self, admission_number: str):
        ...
        return (await self.db.execute(select(IpdPaymentTransaction).where(IpdPaymentTransaction.admission_number == admission_number).order_by(IpdPaymentTransaction.payment_date.desc()))).scalars().all()
    ...
    async def get_billing_dashboard(self):
        ...
        active_adms = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.status != "Discharged"))).scalars().all()
        for adm in active_adms:
            await self.get_or_create_billing(adm.admission_number)
        
        return (await self.db.execute(select(IpdBillingMaster).order_by(IpdBillingMaster.updated_at.desc()))).scalars().all()
    ...
    async def get_patient_active_insurance(self, admission_number: str):
        ...
        patient = (await self.db.execute(select(Patient).where(Patient.patient_uuid == adm.patient_uhid))).scalar_one_or_none()
        if not patient:
            return []
            
        insurances = (await self.db.execute(select(PatientInsurance).where(PatientInsurance.patient_id == patient.id))).scalars().all()
        return [{"provider": i.insurance_provider, "policy": i.policy_number, "type": i.coverage_type} for i in insurances]
    ...
    async def get_visitors(self, admission_number: str):
        from .models import IpdVisitor
        return (await self.db.execute(select(IpdVisitor).where(IpdVisitor.admission_number == admission_number).order_by(IpdVisitor.registered_at.desc()))).scalars().all()
    ...
    async def get_visitor_passes(self, admission_number: str):
        from .models import IpdVisitorPass
        return (await self.db.execute(select(IpdVisitorPass).where(IpdVisitorPass.admission_number == admission_number).order_by(IpdVisitorPass.created_at.desc()))).scalars().all()
    ...
    async def log_visitor_exit(self, log_id: str):
        from .models import IpdVisitorLog
        log = (await self.db.execute(select(IpdVisitorLog).where(IpdVisitorLog.id == log_id))).scalar_one_or_none()
        if log:
    ...
    async def get_visitor_logs(self, pass_number: str):
        from .models import IpdVisitorLog
        return (await self.db.execute(select(IpdVisitorLog).where(IpdVisitorLog.pass_number == pass_number).order_by(IpdVisitorLog.entry_time.desc()))).scalars().all()
    ...
    async def register_mlc_case(self, data: dict):
        ...
        adm = (await self.db.execute(select(IpdAdmissionRecord).where(IpdAdmissionRecord.admission_number == data["admission_number"]))).scalar_one_or_none()
    ...
    async def get_mlc_case(self, admission_number: str):
        ...
        return (await self.db.execute(select(IpdMlcCase).where(IpdMlcCase.admission_number == admission_number))).scalar_one_or_none()

    async def get_all_mlc_cases(self):
        from .models import IpdMlcCase
        return (await self.db.execute(select(IpdMlcCase).order_by(IpdMlcCase.registered_at.desc()))).scalars().all()

    async def update_mlc_case(self, admission_number: str, data: dict):
        from .models import IpdMlcCase
        mlc = (await self.db.execute(select(IpdMlcCase).where(IpdMlcCase.admission_number == admission_number))).scalar_one_or_none()
        if mlc:
            for k, v in data.items():
                if v is not None:
                    setattr(mlc, k, v)
    ...
    async def get_mlc_documents(self, mlc_id: str):
        from .models import IpdMlcDocument
        return (await self.db.execute(select(IpdMlcDocument).where(IpdMlcDocument.mlc_id == mlc_id).order_by(IpdMlcDocument.uploaded_at.desc()))).scalars().all()
    ...
    async def mark_notification_read(self, notif_id: str):
        from .models import IpdSecurityNotification
        notif = (await self.db.execute(select(IpdSecurityNotification).where(IpdSecurityNotification.id == notif_id))).scalar_one_or_none()
        if notif:
            notif.is_read = True
            await self.db.flush()
        return notif
