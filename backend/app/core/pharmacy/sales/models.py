...
class PharmacyWalkInSale(Base):
    ...
    pharmacist_id = Column(UUID(as_uuid=True), ForeignKey("users.id", ondelete="RESTRICT"), nullable=False, index=True)
...
class PharmacySaleItem(Base):
    ...
    batch_id = Column(UUID(as_uuid=True), ForeignKey("drug_batches.id", ondelete="RESTRICT"), nullable=False, index=True)
