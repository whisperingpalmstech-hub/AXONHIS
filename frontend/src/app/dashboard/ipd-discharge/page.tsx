    ...
    <div className="p-6 h-full flex flex-col space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Manage patient readiness, checklists, and documentation</p>
        </div>
        
        <div className="w-1/3">
          <label className="block text-xs font-semibold text-gray-600 mb-1 uppercase tracking-wider">Select Active Admission</label>
          <select 
            value={selectedAdmNo} 
      ...
      {loading && <div className="text-center p-8 text-emerald-600 animate-pulse">Loading Discharge Engine...</div>}
          ...
          <div className="space-y-6">
            <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden">
              ...
              <div className="space-y-3">
                <label className="block text-xs text-gray-500">Planned Discharge Date</label>
                <input 
                  type="datetime-local" 
            ...
            <div className={`p-5 rounded-2xl border shadow-sm ${pendingOrders.length > 0 ? 'bg-red-50 border-red-100' : 'bg-green-50 border-green-100'}`}>
              ...
              {pendingOrders.length > 0 ? (
                ...
                <p className="text-sm text-green-700">All clinical orders are completed.</p>
              )}
            </div>
          </div>
      ...
      {!loading && !selectedAdmNo && (
        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl bg-gray-50">
          ...
          <p className="text-gray-500 font-medium text-lg">Select a patient admission to begin discharge planning</p>
        </div>
      )}
    </div>
  );
