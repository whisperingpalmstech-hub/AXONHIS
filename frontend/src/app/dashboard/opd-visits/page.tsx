  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3 text-slate-800">
            <Bot className="text-indigo-600" size={32} />{t("opdVisits.opdVisitIntelligence")}</h1>
          <p className="text-slate-500 mt-1">{t("opdVisits.aiDrivenPatientIntakeTriageAnd")}</p>
        </div>
      </div>
      ...
      <div className="flex bg-white rounded-lg p-1 shadow-sm w-fit border border-slate-200">
        {[
          { id: "intake", icon: Mic, label: t("opdVisits.tabAiIntake") },
          { id: "queue", icon: Activity, label: t("opdVisits.tabLiveQueue") },
          { id: "questionnaires", icon: FileText, label: t("opdVisits.tabQuestionnaires") },
          { id: "analytics", icon: Activity, label: t("opdVisits.tabAnalytics") }
        ].map(t => (
          <button
            key={t.id}
      ...
      {activeTab === "intake" && (
        <div className="space-y-6">
          <div className="flex items-center gap-12 text-sm font-medium text-slate-400 mb-8 border-b pb-4">
            <div className={intakeStep >= 1 ? "text-indigo-600 flex items-center gap-2" : "flex items-center gap-2"}><span className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center">1</span>{t("opdVisits.patient")}</div>
            <div className={intakeStep >= 2 ? "text-indigo-600 flex items-center gap-2" : "flex items-center gap-2"}><span className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center">2</span>{t("opdVisits.aiIntake")}</div>
            <div className={intakeStep >= 3 ? "text-indigo-600 flex items-center gap-2" : "flex items-center gap-2"}><span className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center">3</span>{t("opdVisits.triageRouting")}</div>
            <div className={intakeStep >= 4 ? "text-emerald-600 flex items-center gap-2" : "flex items-center gap-2"}><span className="w-6 h-6 rounded-full bg-slate-100 flex items-center justify-center">4</span>{t("opdVisits.confirmed")}</div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="col-span-2 space-y-6">
              {intakeStep === 1 && (
                <div className="card p-8 text-center bg-indigo-50/30 border-dashed border-2">
                  <h3 className="text-xl font-bold mb-6">{t("opdVisits.selectPatientForNewVisit")}</h3>
                  <select className="input-field max-w-sm mx-auto mb-6" value={patientId} onChange={e => setPatientId(e.target.value)}>
                    <option value="">-- {t("opdVisits.selectPatient")} --</option>
                    {patients.map(p => <option key={p.id} value={p.id}>{p.first_name} {p.last_name} ({p.patient_uuid})</option>)}
                  </select>
                  <button onClick={startIntake} disabled={loading || !patientId} className="btn-primary">{t("opdVisits.startAiIntakeSession")}</button>
                </div>
              )}
              ...
              {intakeStep === 2 && (
                <div className="card p-6 shadow-md border-indigo-100">
                  <div className="flex justify-between items-center mb-4">
                    <h3 className="font-bold text-lg flex items-center gap-2"><Mic className="text-indigo-500" />{t("opdVisits.patientComplaint")}</h3>
                  ...
                  <textarea 
                    ...
                    placeholder={t("opdVisits.symptomPlaceholder")}
                  ...
                  <button onClick={captureAIComplaint} disabled={loading || !rawComplaint} className="btn-primary w-full flex justify-center gap-2 py-3 text-lg">
                    <Bot />{t("opdVisits.analyzeWithAiEngine")}</button>
                </div>
              )}
              ...
              {intakeStep === 3 && complaintRes && classRes && docRec && (
                <div className="card p-6 shadow-md border-indigo-100 space-y-6">
                  <div>
                    <h3 className="font-bold text-lg flex items-center gap-2 mb-4"><CheckCircle2 className="text-emerald-500" />{t("opdVisits.aiFindings")}</h3>
                    <div className="bg-slate-50 p-4 rounded-xl space-y-4">
                      <div>
                        <span className="text-sm font-bold text-slate-500 uppercase tracking-wider">{t("opdVisits.extractedSymptoms")}</span>
                        <div className="flex gap-2 mt-2">
                          {complaintRes.structured_symptoms?.map((s,i) => <span key={i} className="badge bg-purple-100 text-purple-700">{s}</span>)}
                        </div>
                      </div>
                      <div>
                        <span className="text-sm font-bold text-slate-500 uppercase tracking-wider">{t("opdVisits.icpcClassification")}</span>
                        <div className="flex gap-2 mt-2">
                      ...
                      <div className="flex justify-between items-center bg-white p-3 rounded-lg border">
                        <div>
                          <div className="font-bold text-sm">{t("opdVisits.aiSeverityScore")}</div>
                          <div className="text-xs text-slate-500">{t("opdVisits.basedOnComplaintAnalysis")}</div>
                        </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className={`p-4 rounded-xl border-2 ${classRes.category === 'emergency_opd' ? 'border-rose-500 bg-rose-50' : 'border-amber-500 bg-amber-50'}`}>
                      <span className="text-sm font-bold opacity-70 uppercase">{t("opdVisits.triageResult")}</span>
                    ...
                    <div className="p-4 rounded-xl border border-slate-200 bg-white shadow-sm">
                      <span className="text-sm font-bold text-slate-500 uppercase">{t("opdVisits.recommendedRouting")}</span>
                      <div className="text-lg font-bold mt-1 text-indigo-700">{docRec.recommended_specialty}</div>
                      <div className="text-xs text-slate-500 mt-2">{t("opdVisits.selectDoctor")}:</div>
                  ...
                  <button onClick={finalizeVisit} disabled={loading} className="btn-primary w-full py-3">{t("opdVisits.confirmRoutingAddToQueue")}</button>
                </div>
              )}
              ...
              {intakeStep === 4 && (
                <div className="card p-12 text-center border-emerald-500 border-2 bg-emerald-50">
                  ...
                  <h2 className="text-2xl font-bold text-emerald-800 mb-2">{t("opdVisits.visitCreatedSuccessfully")}</h2>
                  <p className="text-emerald-600 mb-6">{t("opdVisits.patientRoutedTo")}<strong>{currentVisit?.department || "Specialty"}</strong>.</p>
                  <button onClick={resetIntake} className="btn-primary bg-emerald-600 hover:bg-emerald-700">{t("opdVisits.startNextIntake")}</button>
                </div>
              )}
            </div>
            ...
            <div className="col-span-1">
              <div className="card p-5 sticky top-6">
                <h3 className="font-bold flex items-center gap-2 mb-4"><Activity size={16} />{t("opdVisits.patientClinicalContext")}</h3>
                
                {contextSnap ? (
                  <div className="space-y-4 text-sm">
                    <div className="bg-slate-50 p-3 rounded-lg border">
                      <div className="text-xs font-bold text-slate-500 uppercase">{t("opdVisits.summary")}</div>
                      <div className="font-medium mt-1">{contextSnap.context_summary}</div>
                    </div>
                    
                    <div>
                      <div className="text-xs font-bold text-slate-500 uppercase mb-2">{t("opdVisits.previousDiagnoses")}</div>
                      ...
                      ) : <p className="text-slate-400 text-xs italic">{t("opdVisits.noHistory")}</p>}
                    </div>
                  </div>
                ) : (
                  <div className="text-center p-8 text-slate-400 text-sm">{t("opdVisits.selectAPatientToPullContextEng")}</div>
                )}
              </div>
            </div>
          </div>
      ...
      {activeTab === "queue" && (
        <div className="card p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold">{t("opdVisits.intelligentQueue")}</h2>
            <button onClick={loadQueue} className="btn-secondary"><RefreshCw size={14} className="mr-2" />{t("opdVisits.refresh")}</button>
          </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-600 border-b">
                <tr>
                  <th className="py-3 px-4">{t("opdVisits.visitId")}</th>
                  <th className="py-3 px-4">{t("opdVisits.patient")}</th>
                  <th className="py-3 px-4">{t("opdVisits.type")}</th>
                  <th className="py-3 px-4">{t("opdVisits.priority")}</th>
                  <th className="py-3 px-4">{t("opdVisits.status")}</th>
                  <th className="py-3 px-4">{t("opdVisits.doctor")}</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                ...
                {queue.length === 0 && <tr><td colSpan={6} className="text-center py-8 text-slate-400">{t("opdVisits.queueIsEmpty")}</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-4 gap-6">
            <div className="card p-5 bg-gradient-to-br from-indigo-500 to-indigo-600 text-white">
              <div className="text-indigo-100 text-sm font-medium mb-1">{t("opdVisits.totalVisitsToday")}</div>
              <div className="text-4xl font-black">{analytics.total_visits}</div>
            </div>
            <div className="card p-5">
              <div className="text-slate-500 text-sm font-medium mb-1">{t("opdVisits.emergencyTriage")}</div>
              <div className="text-3xl font-black text-rose-600">{analytics.emergency_visits}</div>
            </div>
            <div className="card p-5">
              <div className="text-slate-500 text-sm font-medium mb-1">{t("opdVisits.priorityTriage")}</div>
              <div className="text-3xl font-black text-amber-500">{analytics.priority_visits}</div>
            </div>
            <div className="card p-5">
              <div className="text-slate-500 text-sm font-medium mb-1">{t("opdVisits.routine")}</div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="card p-6">
              <h3 className="font-bold text-lg mb-4">{t("opdVisits.topSpecialtiesRecommended")}</h3>
              <div className="space-y-4">
                {analytics.top_specialties?.map((s: any, i: number) => (
                  <div key={i} className="flex justify-between items-center">
                    <span className="font-medium text-slate-700">{s.specialty}</span>
                    <span className="badge bg-indigo-50 text-indigo-700 font-bold">{s.count} {t("opdVisits.visitsCount")}</span>
                  </div>
                )) || <div className="text-slate-400 italic">{t("opdVisits.noData")}</div>}
              </div>
            </div>
          </div>
        </div>
