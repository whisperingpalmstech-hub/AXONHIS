    ...
    <div className="p-8 max-w-7xl mx-auto animate-in slide-in-from-right-4 duration-500">
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="mt-2 text-slate-500">Global application configurations, features, and environment variables.</p>
        </div>
      ...
      <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center text-slate-500">No settings found.</div>
        ) : (
            ...
            {configs.map((c) => (
              <div key={c.key} className="p-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:bg-slate-50 transition-colors group">
                <div className="flex-1">
                  ...
                  <p className="text-sm text-slate-500 max-w-2xl">{c.description || "No description provided."}</p>
                  <p className="text-xs text-slate-400 mt-2">Last updated: {new Date(c.updated_at).toLocaleString()}</p>
                </div>
