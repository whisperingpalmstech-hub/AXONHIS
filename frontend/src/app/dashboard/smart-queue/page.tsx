...
export default function SmartQueuePage() {
  ...
  const addPatientToQueue = async (priority: string) => {
    ...
    try {
      ...
      alert(`Patient added to queue as ${priority.toUpperCase()}`);
    } catch (e: any) {
      setError(e.response?.data?.detail || e.message);
    }
    setLoading(false);
  ...
  const fetchSignage = async () => {
    if (!queueMaster) {
      setError("Please initialize a queue in the Orchestrator first.");
  ...
  return (
    <div className="p-8 max-w-7xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-3 text-slate-800">
            ...
            Smart Queue & Patient Flow
          </h1>
          <p className="text-slate-500 mt-1">Enterprise flow orchestration, digital signage, and tokenless routing.</p>
        </div>
      </div>
      ...
      <div className="flex bg-white rounded-lg p-1 shadow-sm w-fit border border-slate-200">
        {[
          { id: "orchestrator", icon: MonitorPlay, label: "Queue Orchestrator" },
          { id: "signage", icon: MonitorPlay, label: "Digital Signage" },
          { id: "wayfinding", icon: Compass, label: "Digital Wayfinding" },
          { id: "ai_crowd", icon: TrendingUp, label: "Crowd Analytics (AI)" }
        ].map(t => (
          <button
            key={t.id}
            onClick={() => {
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="card p-6 border-indigo-100">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                1. Central Queue Master
              </h3>
              <div className="space-y-4">
                <div>
                  <label className="text-sm font-medium text-slate-600">Select Department Segment</label>
                ...
                <button onClick={initializeQueue} disabled={loading} className="btn-primary w-full">Start Orchestration Instance</button>
              </div>

              {queueMaster && (
                <div className="mt-6 p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
                  <div className="font-bold text-emerald-800">Queue Engine Active</div>
                  <div className="text-sm text-emerald-700 mt-2 flex justify-between">
                    <span>Target: {queueMaster.department} / {queueMaster.room_number}</span>
                    <span className="font-black">Length: {queueMaster.current_length}</span>
                  </div>
                </div>
              )}
              ...
              {routingRec && (
                <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg text-sm">
                  <span className="font-bold text-blue-800 block mb-1 flex items-center gap-2"><ArrowRight size={14}/> Doctor Availability Routing Recs:</span>
                  Best Room: <span className="font-bold">{routingRec.room_number}</span> (Queue length: {routingRec.length})
                </div>
              )}
            </div>
            ...
            <div className="card p-6 border-slate-200 opacity-90">
              <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
                2. Simulate Flow & Recovery
              </h3>
              {!queueMaster ? (
                <div className="text-center p-8 text-slate-400">Initialize Orchestrator First</div>
              ) : (
                <div className="space-y-6">
                  <div>
                    <select className="input-field mb-2" value={mockPatientId} onChange={e=>setMockPatientId(e.target.value)}>
                      <option value="">-- Simulation Patient --</option>
                      {patients.map(p => <option key={p.id} value={p.id}>{p.first_name} {p.last_name}</option>)}
                    </select>
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <button onClick={() => addPatientToQueue('emergency')} className="btn-primary bg-rose-600 hover:bg-rose-700">+ Add Emergency</button>
                      <button onClick={() => addPatientToQueue('priority')} className="btn-primary bg-amber-600 hover:bg-amber-700">+ Add Priority</button>
                      <button onClick={() => addPatientToQueue('walk_in')} className="btn-secondary col-span-2">+ Add Walk-in (Standard)</button>
                    </div>
                  </div>

                  <div className="pt-4 border-t border-slate-200">
                    <h4 className="font-bold text-sm mb-2 text-slate-600 flex items-center gap-2"><UserMinus size={14}/> Recovery Logic Test</h4>
                    <p className="text-xs text-slate-500 mb-2">If patient misses call, moving them to standby penalization state without deleting tokens.</p>
                    <button onClick={() => alert('Moved back 3 slots via Smart Recalculation!')} className="btn-secondary w-full">Trigger Patient Missed Handling</button>
                  </div>
                </div>
              )}
            </div>
      ...
      {activeTab === "signage" && signageData && (
        <div className="bg-slate-900 border-[8px] border-slate-800 rounded-xl p-8 text-white min-h-[400px] shadow-2xl relative overflow-hidden">
            ...
            <div className="col-span-2">
              <div className="bg-white/5 rounded-2xl p-6 border border-white/10 h-full flex flex-col justify-center items-center text-center">
                ...
                {signageData.current_patient ? (
                  ...
                  <div className="text-xl text-slate-500">Awaiting Patient</div>
                )}
              </div>
            </div>
                ...
                {signageData.next_patients.length > 0 ? signageData.next_patients.map((np, i) => (
                  <div key={i} className="bg-white/5 border border-white/5 rounded-xl p-4 flex justify-between items-center hover:bg-white/10 transition-colors">
                    ...
                    <div className="text-right">
                      {np.priority_level === 'emergency' && <span className="text-rose-400 text-xs font-bold uppercase block mb-1">Emergency Call</span>}
                      <span className="text-slate-400 text-sm">Est Wait: <span className="text-white font-bold">{signageData.avg_wait_time_min * (i + 1)} min</span></span>
                    </div>
                  </div>
                )) : (
                  <div className="p-8 text-center border border-dashed border-slate-700 rounded-xl text-slate-500">Queue is empty</div>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center text-slate-400 text-sm relative z-10">
            <div>Total Currently Waiting: <strong className="text-white ml-2">{signageData.queue_length}</strong></div>
            <div className="flex items-center gap-2"><Bell size={14} className="text-amber-400" /> Patients will be notified via SMS 10 minutes prior.</div>
          </div>
        </div>
      )}
      ...
      {activeTab === "wayfinding" && (
        <div className="card p-6 max-w-2xl mx-auto">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold">Digital Route Guidance Engine</h2>
            <p className="text-slate-500">Search room to fetch indoor navigation nodes</p>
          </div>
          ...
          {wayfindingInstr && (
            <div className="bg-indigo-50 border-l-4 border-indigo-600 p-6 rounded-r-lg shadow-sm">
              <h4 className="font-bold text-indigo-900 flex items-center gap-2 mb-2"><Map size={16} /> Directions map generated</h4>
      ...
      {activeTab === "ai_crowd" && (
        <div className="card p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold flex items-center gap-2"><TrendingUp className="text-indigo-500"/> OP Crowding Prediction</h2>
            <button onClick={runCrowdAI} disabled={loading} className="btn-primary">Execute Forecasting Model</button>
          </div>
            ...
            <div className="space-y-6">
              <div className="bg-gradient-to-r from-slate-800 to-indigo-900 rounded-xl p-8 text-white grid grid-cols-3 gap-6 shadow-lg">
                <div className="border-r border-white/20">
                  <div className="text-slate-400 text-sm mb-1 uppercase tracking-wider font-bold">Predicted Block Peak</div>
                ...
                <div className="border-r border-white/20 pl-6">
                  <div className="text-slate-400 text-sm mb-1 uppercase tracking-wider font-bold">Inflow Est</div>
                  <div className="text-4xl font-black text-emerald-400 drop-shadow-sm">
                    {crowdAI.predicted_inflow_count} <span className="text-lg text-emerald-400/50">pts</span>
                  </div>
                </div>
                <div className="pl-6">
                  <div className="text-slate-400 text-sm mb-1 uppercase tracking-wider font-bold">Est Wait Surge</div>
                  <div className="text-4xl font-black text-rose-400 drop-shadow-sm">
                    45 <span className="text-lg text-rose-400/50">min avg</span>
                  </div>
                </div>
              </div>
