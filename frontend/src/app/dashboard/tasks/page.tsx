  ...
  return (
    <div className="flex-1 h-screen overflow-hidden flex flex-col bg-[var(--bg-secondary)] relative">
      <div className="flex z-10 p-6 md:p-8 justify-between items-start gap-4 flex-col sm:flex-row shadow-sm bg-[var(--bg-primary)] border-b border-[var(--border)]">
        <div>
          <h1 className="text-3xl font-extrabold text-[var(--text-primary)] tracking-tight">Task & Care Execution</h1>
          <p className="mt-1 text-sm text-[var(--text-secondary)]">Manage clinical workflows, nursing tasks, and administrative duties.</p>
        </div>
          ...
          {activeTab === "queue" && (
            <div className="flex flex-wrap gap-4 items-center bg-white p-4 rounded-xl border border-[var(--border)] shadow-sm">
              <div className="flex-1 min-w-[200px] relative">
                ...
                <input
                  type="text"
                  placeholder="Search tasks by patient or description..."
              ...
              >
                ...
                <option value="radiology_tech">Radiology Tech</option>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl border border-[var(--border)] p-12 text-center shadow-sm">
              ...
              <p className="text-[var(--text-secondary)] mt-1 max-w-sm mx-auto">
                {activeTab === "my_tasks" ? "You have no active tasks assigned to you right now." : "No tasks found in the queue matching your filters."}
              </p>
            </div>
          ) : (
                  ...
                  <div key={task.id} className="bg-white rounded-2xl border border-[var(--border)] overflow-hidden shadow-sm hover:shadow-md transition-shadow group flex flex-col">
                    <div className="p-5 border-b border-slate-100 flex-1">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: typeInfo.color }}>{typeInfo.label}</p>
                      ...
                      <div className="space-y-3 mt-4">
                        ...
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium text-slate-900 capitalize">{task.assigned_to_role?.replace('_', ' ') || 'Any'}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                        ...
                        {task.schedule_interval && (
                          <div className="flex justify-between text-sm bg-slate-50 p-2 rounded border border-slate-100">
                            ...
                            <span className="font-medium text-blue-600">Every {task.schedule_interval} mins</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-4 border-t border-[var(--border)] flex gap-3 justify-end items-center">
                      {task.status === "PENDING" && (
                        ...
                        >
                          <User className="w-4 h-4" /> Pick Up Task
                        </button>
                      )}
                      ...
                      {['COMPLETED', 'CANCELLED'].includes(task.status) && (
                        <div className="text-sm font-medium text-slate-500 w-full text-center">
                          {task.status === 'COMPLETED' ? 'Completed by user' : 'Cancelled'}
                        </div>
                      )}
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-5">
              <div className="bg-blue-50 text-blue-800 p-4 rounded-xl border border-blue-100 flex gap-4 items-start">
                ...
                <div>
                  ...
                  <p className="text-sm text-blue-600 mt-1 opacity-80">Log your execution details below.</p>
                </div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Execution Notes (Optional)</label>
                <textarea
                  value={completeNotes}
                  onChange={e => setCompleteNotes(e.target.value)}
                  placeholder="e.g., Patient tolerated procedure well. Sample collected from left arm."
            ...
            <div className="p-6 bg-slate-50 border-t border-[var(--border)] flex justify-end gap-3">
              <button onClick={() => { setShowCompleteModal(false); setSelectedTask(null); }} className="px-5 py-2.5 font-semibold text-slate-600 hover:bg-slate-200 rounded-xl transition-colors">Cancel</button>
              <button
